...
def main_page() :
    ...
    with register_db.cursor(pymysql.cursors.DictCursor) as cursor :
        ...
        if search_query:
            cursor.execute("SELECT datetime, project_title, project_image_name, project_OneLine_info, user_email FROM project_post WHERE project_title LIKE %s LIMIT %s OFFSET %s", (search_query, per_page, (page-1)*per_page))
        else:
            # 검색어가 없을 경우 모든 데이터 반환
            cursor.execute("SELECT datetime, project_title, project_image_name, project_OneLine_info, user_email FROM project_post LIMIT %s OFFSET %s", (per_page, (page-1)*per_page))
